module.exports = {
    ...
    plugins: [
        ...
        [
            ...
            {
                "prepareCmd": "sed -i -E 's/^version \\\".*\\\"/version \\\"${nextRelease.version}\\\"/g' fxmanifest.lua && sed -i -E 's/\\\"version\\\": \\\".*\\\"/\\\"version\\\": \\\"${nextRelease.version}\\\"/g' web/package.json && sed -i -E 's/\\\"version\\\": \\\".*\\\"/\\\"version\\\": \\\"${nextRelease.version}\\\"/g' .release/package.json && cd web && pnpm install && pnpm build && cd .. && zip -r ps-hud.zip . -x \"web/node_modules/*\" \"web/src/*\" \"web/public/*\" \"web/scripts/*\" \"web/*.json\" \"web/*.config.ts\" \"web/*.config.js\" \"svelte-source/*\" \".git/*\" \".github/*\" \".release/*\" \"node_modules/*\" \"version\" \"package.json\" \"package-lock.json\" \"CHANGELOG.md\" \".vscode/*\""
            }
        ],
        [
            "@semantic-release/git",
            {
                assets: ["package.json", "web/package.json", ".release/package.json", "fxmanifest.lua", "CHANGELOG.md", "html/**/*"],
